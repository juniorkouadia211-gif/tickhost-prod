...
export const useStore = create<AppState>((set, get) => ({
  ...
  fetchStats: async (eventId?: string | number) => {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        // En cas d'erreur d'autorisation, on arrête de spammer mais on ne déconnecte pas forcément immédiatement 
        // pour laisser une chance au rafraîchissement ou à la session staff d'expirer proprement
      ...
      // Éviter de logguer les erreurs d'annulation normales
      if (err.name !== 'AbortError') {
