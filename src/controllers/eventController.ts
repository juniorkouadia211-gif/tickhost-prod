...
// Vérification signature webhook partenaire
...
export const getEvents = (req: AuthRequest, res: Response) => {
  try {
    ...
    if (tenantSlug) {
      const event = db.prepare("SELECT * FROM events WHERE slug = ? AND status = 'published'").get(tenantSlug) as any;
      ...
      jsonFields.forEach(field => {
        if (event[field]) {
          try { event[field] = JSON.parse(event[field]); }
          catch { event[field] = arrayFields.includes(field) ? [] : {}; }
        } else {
    ...
    const parsedEvents = events.map(e => {
      ...
      jsonFields.forEach(field => {
        if (e[field]) {
          try { e[field] = JSON.parse(e[field]); }
          catch { e[field] = arrayFields.includes(field) ? [] : {}; }
        } else {
...
export const createEvent = (req: AuthRequest, res: Response) => {
  ...
  if (!userExists) return res.status(401).json({ success: false, error: 'Session expirée. Veuillez vous reconnecter.' });

  try {
    const eventId = uuidv4();
    let slug = req.body.slug
      ? req.body.slug.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '')
      : name.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');

    const existingSlug = db.prepare('SELECT id FROM events WHERE slug = ?').get(slug);
    if (existingSlug) {
      return res.status(400).json({ success: false, error: 'Ce lien URL est déjà pris.' });
    }
...
export const updateEvent = (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    `).run(
      ...
      safe(options, true), safe(info_options, true), safe(info_sections, true),
      safe(payment_modes, true), safe(status),
      safe(bg_intensity), safe(bg_opacity), safe(show_logo_instead_of_name),
      safe(logo_url_main), safe(partner_billing_enabled),
      safe(organizer_name), safe(gallery_title),
      safe(home_options, true), safe(partners, true),
      id
    );
...
export const addPartner = (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!subscription) {
      ...
      if ((partnerCount?.count || 0) >= freeLimit) {
        return res.status(400).json({
          success: false,
          error: 'Quota gratuit atteint. Souscrivez à un abonnement premium.',
...
export const deletePartner = (req: AuthRequest, res: Response) => {
  try {
    ...
    if (partner) syncEventPartners(partner.event_id);
    res.json({ success: true });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err.message });
  }
  ...
  try {
    ...
    db.transaction(() => {
      ...
      const result = db.prepare(`
        ...
        AND status = 'unused'
      `).run(id);
      logger.info('Événement terminé', { eventId: id, ticketsExpired: result.changes, byUser: userId });
    })();
